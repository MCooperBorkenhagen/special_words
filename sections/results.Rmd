...
We next consider whether there might be more commonality across resources than the above data suggest. Although they differ in the exact choice of words, perhaps they agree on which types of words should be identified in terms of the properties of words that cause them to require special attention. In order to examine this possibility we looked at each set of words in terms of a range of properties known to be important in reading development: frequency, age-of-acquisition, consistency, imageability, and length (number of letters and number of syllables). 
...
table_desc %>% 
  ...
  apa_table(caption = "Descriptive Data for Frequency for Instructional Sources",
...
As with word frequency, the measures of these additional word-level variables were standardized in the TASA corpus (levels 1-3) in the same way that frequency was standardized (described previously). Table 8 provides a description of what low values on each scale can be interpreted as, and example words. Low values are described for these variables because words in all distributions in the data described tend to be on the low end of the scale for each variable (and tending to be below the mean on the variable in the TASA corpus). 
...
```{r profileFigureLexical, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='Point estimates and standard errors for a range of word variables within sources are shown. Means are calculated over standardized measures within TASA, therefore the position of the point for a source for a variable can be interpreted as the average difference from the mean of all words within TASA (i.e., 0 shown with a red dashed line) for that variable in terms of standard deviation units. The mean on a given variable across all sources is shown as a grey dotted line just for convenience. FP = Fountas and Pinnell. Estimates were derived using t.test() in base R.', fig.width=7}
