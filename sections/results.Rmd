...
In answer to the question posed at the beginning of this section, Figure 5 indicates that the words in Wonders, Fry, Dolch, and Fountas & Pinnel have similar frequencies even though they are largely different words. As other analyses have indicated, the Kilpatrick words differ the most from the others, but the Fundations words stand out because they include fewer of the words in each frequency band.  
...
As with word frequency, the measures of these additional word-level variables were standardized in the TASA corpus (levels 1-3) in the same way that frequency was standardized (described previously). Table 8 provides a description of what low values on each scale can be interpreted as, and a few examples. Low values are described because words in all distributions in the data described tend to be on the low end of the scale for each variable (and below the mean on the variable in the TASA corpus). 
