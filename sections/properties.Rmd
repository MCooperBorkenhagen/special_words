...
It is possible that the words across resources might be different while the underlying properties of the words might not vary. If this is the case then variability in the words selected for instruction would be less meaningful; Students would have similar experiences in this form of instruction by virtue of the underlying language structures and properties associated with these words. In order to examine this possibility we will look at each set of words in terms of a range of properties known to be important in reading development: frequency, age-of-acquisition, consistency, imageability, number of letters, and number of syllables.

## Frequency
Data from the Educator's Word Frequency Guide from Touchstone Applied Science Associates [@Zeno1995] are used in order to examine word frequency^["See supplement for additional analyses that consider other sources of word frequency"]. Words from the Educator's Word Frequency Guide (TASA by convention) were subset for grade levels K through 3 in order to match the grade levels associated with our sample of instructional words. This corpus exhibits a mean frequency value of `r frequency_by_corpus(all_lists) %>% filter(Corpus == "TASA") %>%  pull(M)` (SD = `r frequency_by_corpus(all_lists) %>% filter(Corpus == "TASA") %>%  pull(SD)`, max = `r frequency_by_corpus(all_lists) %>% filter(Corpus == "TASA") %>%  pull(max)`). See the beginning of the section on research question RQ1 for summary statistics for this corpus for each resource.
