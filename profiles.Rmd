...
```{r setup_profiles, include=FALSE}
source('frequency_models.R')
source('lexical_models.R')
```

# The properties of words across sources
One method of capturing the differences across sources concerns the characteristics of the words that they contain. There are several ways of achieving such a comparison. One is doing so directly: one can observe the relative distribution of frequency, word length, etc. across two (or more) instructional lists. However, this sort of direct comparison evades useful description given that it isn't clear how a given distribution sits relative to the characteristics of words that children (who would consume such instructional resources) might be exposed to in their language (and reading) experience more generally. This section reports on a method to facilitate comparison of the distribution of the properties of words in an instructional source relative to some relevant benchmark.
...
```
Word frequency is a property that differs based on the source from which it is drawn. A word's frequency in a corpus assembled from transcripts of spoken language interactions with children will have a different frequency (and be drawn from a different distribution) than that word's frequency in a corpus of adult fiction. In order to capture something about this variability, four different sources of word frequency are considered here: the Wisconsin Children's Book Corpus, the CHILDES database, the TASA norms, and the Corpus of Contemporary American English. Each represents a different type of perspective from a reading development standpoint.

The Wisconsin Children's Book Corpus contains texts commonly consumed by pre-readers (up to the age of 5). CHILDES captures something about children's spoken language environment (the data used here are from transcripts for children in the US up to the age of 5 years). TASA captures information about words in literature children are commonly exposed to in grade school, and the Corpus of Contemporary American English captures information about adult texts (in this case from fiction books only).

For all the frequency data, rank frequencies are used; low values indicate high frequencies. While rank frequencies exhibit a uniform distribution (one word per rank), this type of frequency measure avoids the skew found in raw frequencies - which can make interpretation more difficult. A table of descriptive statistics across corpora and sources are shown below.

```{r descriptivesFreqSource, echo=FALSE, warning=FALSE, message=FALSE}

sources = desc_by_var(Zs, 'wcbc_rank', combine_ = T)$source
wcbc = desc_by_var(Zs, 'wcbc_rank', combine_ = T)$var
...
data.frame(cbind(sources, wcbc, tasa, coca, childes)) %>% 
  mutate(sources = tools::toTitleCase(sources))  %>% 
  rename(Source = sources,
         WCBC = wcbc,
         TASA = tasa,
         COCA = coca,
         CHILDES = childes) %>% 
  apa_table(caption = 'Descriptive Statistics of Rank Frequency for each Source across Four Corpora', 
            note = 'Means and standard deviations (in parentheses) are shown over Z transformed rank frequency values for all words in a given instructional source. Frequencies are transformed based on all words in a given corpus before subsetting by instructional source. As a result, the mean rank frequency shown represents how far from the mean of all words in a given corpus (zero) the distribution within an instructional source is.')



```

In order to make direct comparisons in frequency across instructional resources, frequency values were standardized within the language corpus from which it is drawn. After standardization, values for frequency were subset for each instructional source. The result of this process is that for a given source, the standardized values represent their position in the standardized distribution for that corpus, where the corpus as a whole becomes a reference distribution, and dues to standardization the mean of this distribution is always zero and the standard deviation is always one. This is useful when one considers the characteristics of a given text corpus from which the frequency distribution is constructed (child-directed text, child-directed speech, adult-directed text, etc.). It results from this process that the mean frequency _within_ a specific source will differ from the distribution of the corpus as a whole (in terms of its average, its spread, etc.), and these differences allow the observer to make a comparison to the reference corpus in a direct way that is useful.

The figure below shows the distribution of frequencies for each of these four corpora across the five instructional resources. A black dotted line is included at zero on the y-axis to represent the mean frequency for all language corpora (the mean is always zero due to standardization). Grey points within each panel are individual observations (words) within each source for a given value for standardized rank frequency. The colored points represent the mean for a source on a given frequency, and the colored bar around that mean represents the standard deviation for that source on that frequency. The red dashed line represents the mean across all frequencies for that instructional source, which facilitates a direct comparison across instructional sources (e.g., Kilpatrick on average contains much lower frequency words than the rest of the sources, as indicated by a higher red dashed line).
...
Zs %>% 
  ...
  geom_hline(data = hlines, aes(yintercept = varmean), color = 'red', linetype = 'dashed', alpha = .5) +
  theme_bw() +
...
Zs %>% 
  ...
  geom_hline(data = hlines, aes(yintercept = sourcemean), color = 'grey45', linetype = 'dotted') +
  scale_color_manual(values = COLORS_SOURCE) +
  geom_point(size = 2) +
  geom_line(color = 'grey') +
